...
IUSE="dv matroska mpegts mythtv ogg pcm X"
...
pkg_setup() {
	...
	if ! built_with_use media-video/mplayer real; then
		elog "MPlayer has been built without the 'real' USE flag enabled."
		elog "This is optional as it may require a full X11 installation (depending on the arch)."
		elog "However it is strongly recommended as it enables this program's"
		elog "ability to process Real Media video and audio formats."
		echo
	fi
	if ! use amd64 || ! use ia64; then
		if ! built_with_use media-video/mplayer win32codecs; then
			eerror "Mplayer is missing 'win32codecs' support. Please add"
			eerror "'win32codecs' to your USE flags, and re-emerge media-video/mplayer"
			ewarn "USE flag 'win32codecs' is mandatory for all arches except amd64 and ia64"
			die "Mplayer needs 'win32codecs' support"
		fi
	else
		if ! built_with_use media-video/mplayer win32codecs; then
			elog "MPlayer has been built without the 'win32codecs' USE flag enabled."
			elog "This is optional as 'win32codecs' may cause binary incompatibilities"
			elog "on some 64-bit systems. However it is strongly recommended it is enabled"
			elog "as it drastically increases this program's supported video and audio formats"
			echo
		fi
	fi
}
